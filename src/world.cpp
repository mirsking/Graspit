...
World::World(QObject *parent, const char *name, IVmgr *mgr) : QObject(parent,name){
  ...
  // DLR
  mBtDynamicsWorld->setGravity(btVector3(5,0,5));
  //BARRETT
...
btBodyMap.insert(btBodyPair(newLink,body));
...
World::addRobot(Robot *robot, bool addToScene) {
  ...
  for (int f=0; f <robot->getNumChains(); f++) {
    ...
    // calcualte chain rotation, and to get base z,x,y in the frame of world frame (palm origin frame)
    ...
    for (int l=0; l <numberLinks; l++) {
      ...
      if (djtype == DynJoint::REVOLUTE) {
        ...
        if (l == 0) {  // palm and link
          newjoint = new btHingeConstraint(*(btbase) , *(btcurrentlink) , chainpos, pivot2,  baseaxis, linkpaxis);
        } else {
          ...
          newjoint = new btHingeConstraint(*(btprevlink), *(btcurrentlink),
                                           btVector3(0 , 0 , 0), pivot2, btVector3(0 , 0 , 1), linkpaxis);
        }
        ...
        //set the constraint frameB: which is the joint0 x,y,z coordinates in new frame(after 2 transformation T1,T2)
        ...
        if (l == 0) {  // connect with palm ,frameinA: palm
          ...
          newjoint = new btGeneric6DofConstraint(*(btbase) , *(btcurrentlink), frameInA, frameInB, false);
        } else {  // privous one is not palm, frameinA: previous link 
          frameInA.setOrigin(btVector3(0, 0, 0));
          frameInA.getBasis().setValue(1, 0, 0,
                                       0, 1, 0,
                                       0, 0, 1);
          newjoint = new btGeneric6DofConstraint(*(btprevlink), *(btcurrentlink), frameInA, frameInB, false);
        }
        ...
        // the other one is the joint1 z axis in the frame of joint0 (x or y),set the left one limit=0,0
